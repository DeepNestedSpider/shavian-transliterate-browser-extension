...
const i18nGlobalRadio = document.getElementById('i18nGlobal') as HTMLInputElement;
...
async function loadSettings(): Promise<void> {
  try {
    ...
    if (currentMode === LanguageCheckMode.HtmlLang) {
      ...
      // Default to I18nGlobal if no setting is found, it's invalid, or it's the default 
      i18nGlobalRadio.checked = true;
    }
...
async function saveSettings(): Promise<void> {
  ...
  } else if (i18nGlobalRadio.checked) {
    
    selectedMode = LanguageCheckMode.I18nGlobal;
  } else {
 ...
 * - When the transliteration toggle changes, save the new settings. 
 */
document.addEventListener('DOMContentLoaded', loadSettings);
saveButton.addEventListener('click', saveSettings);
transliterationToggle.addEventListener('change', saveSettings);
